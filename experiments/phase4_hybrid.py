#!/usr/bin/env python3
"""Phase 4.4 — hybrid per-segment routing (AGENTKV_SPEC.md §4.4).

Extends `phase4_kv_evict.py`'s naive/append_only/kv_evict sweep with a fourth
policy, `hybrid` (per-segment keep-verbatim/evict/summarize routing — see
`policies/hybrid.py`'s module docstring for the routing heuristic and why it
replaces spec's suggested "predicted future attention mass" signal, which §4.2
already found infeasible on this hardware). Same per-request harness, same
theory check, as every other Phase 1-4.1 measurement. Writes to fresh
`phase4_hybrid_*` output files rather than appending to `phase4_kv_evict_*`
(`RecordCollector.flush` appends to an existing file — reusing those names
would silently mix a 3-policy run with a 4-policy one in the same parquet).

Usage (inside the WSL venv):
    python experiments/phase4_hybrid.py --trajectories traj-000 --max-steps 40
    python experiments/phase4_hybrid.py   # full sweep, all trajectories
"""
...
POLICY_ORDER = ["naive", "append_only", "kv_evict", "hybrid"]
...
def run_trajectory(
    *,
    ...
    """Same shape as `phase4_kv_evict.py`'s `run_trajectory` — duplicated
...
def main() -> None:
    ...
    try:
        with VLLMEngine(config, startup_timeout_s=600.0) as engine:
            ...
            for i, trajectory in enumerate(trajectories):
                ...
                # Interleaved A/B/C/D (spec §0.2, extended to N policies):
    ...
    step_collector.flush(args.out_dir / "phase4_hybrid_steps.parquet")
    event_collector.flush(args.out_dir / "phase4_hybrid_events.parquet")
    ...
    out_png = args.out_dir / "phase4_hybrid_comparison.png"
    plot_policy_comparison(runs, out_png)  # type: ignore[arg-type]
    print(f"Wrote {out_png}")

    out_summary = args.out_dir / "phase4_hybrid_gate4_summary.md"
