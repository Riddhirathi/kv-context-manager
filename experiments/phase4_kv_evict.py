#!/usr/bin/env python3
"""Phase 4.1 — KV-space eviction baseline (AGENTKV_SPEC.md §4.1).

Extends `phase2_layout.py`'s naive-vs-append_only sweep with a third policy,
`kv_evict` (StreamingLLM-style sink+window deletion, no summarization).
Measured through the exact same per-request harness as every other policy in
this project — see `policies/kv_evict.py`'s module docstring for why "zero
re-prefill" is not expected to show up as such in these numbers, and why
that's reported honestly here rather than avoided.

Usage (inside the WSL venv):
    python experiments/phase4_kv_evict.py --trajectories traj-000 --max-steps 40
    python experiments/phase4_kv_evict.py   # full sweep, all trajectories
"""
...
POLICY_ORDER = ["naive", "append_only", "kv_evict"]
...
def run_trajectory(
    *,
    ...
    """Same shape as `phase2_layout.py`'s `run_trajectory` (see there for why
    `prev_prompt_ids` threads across trajectory/policy runs rather than
    resetting to `[]`) — duplicated rather than imported, matching this
    project's convention of self-contained experiment scripts."""
...
def write_gate4_prefill_summary(
    results_by_policy: dict[str, list[TrajectoryRunResult]],
    out_path: Path,
) -> str:
    """Prefill-cost half of what Gate 4's Pareto plot needs — task success
    is a separate axis (spec §4.5), not measured here."""
    by_policy_by_traj = {
    ...
    lines = [
        "# Phase 4.1 — kv_evict prefill-cost summary (vs. naive, append_only)",
        "",
    ...
    for policy in POLICY_ORDER:
        if policy == "naive":
            continue
        totals = [float(by_policy_by_traj[policy][t].total_prefill_tokens) for t in shared_ids]
        reductions_pct = [
            100.0 * (n - a) / n for n, a in zip(naive_totals, totals, strict=True) if n > 0
        ]
        test = wilcoxon_signed_rank(naive_totals, totals)
        lines.append(
            f"- Prefill-token reduction, {policy} vs. naive: {fmt(reductions_pct)}%"
        )
        lines.append(
    ...
    lines.append("Per-trajectory detail:")
    lines.append("")
    header = "| trajectory | " + " | ".join(f"{p} prefill" for p in POLICY_ORDER) + " |"
    lines.append(header)
    lines.append("|" + "---|" * (len(POLICY_ORDER) + 1))
    for t in shared_ids:
        cells = [str(by_policy_by_traj[p][t].total_prefill_tokens) for p in POLICY_ORDER]
...
def main() -> None:
    ...
    try:
        with VLLMEngine(config, startup_timeout_s=600.0) as engine:
            ...
            for i, trajectory in enumerate(trajectories):
                ...
                # Interleaved A/B/C (spec §0.2, extended to 3 policies):
                # rotate which policy runs first/second/third per trajectory.
                order = POLICY_ORDER[i % 3 :] + POLICY_ORDER[: i % 3]
    ...
    step_collector.flush(args.out_dir / "phase4_kv_evict_steps.parquet")
    event_collector.flush(args.out_dir / "phase4_kv_evict_events.parquet")
    ...
    out_png = args.out_dir / "phase4_kv_evict_comparison.png"
    plot_policy_comparison(runs, out_png)  # type: ignore[arg-type]
    print(f"Wrote {out_png}")

    out_summary = args.out_dir / "phase4_kv_evict_summary.md"
    summary_text = write_gate4_prefill_summary(results_by_policy, out_summary)
    print()
    print(summary_text)
